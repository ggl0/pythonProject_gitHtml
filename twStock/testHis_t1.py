...
def testBBands(num, macdList, bList, h_p, l_p, c_p, o_p, h_v, m_v, l_v):
...
def testBBands_1(num, macdList, bList, h_p, l_p, c_p, o_p, h_v, m_v, l_v):
    for i in range(len(h_v)):
        if i == 0:
            if ((h_p[i] - l_p[i]) / o_p[i]) > 0.01:
                if h_p[i] >= h_v[i] >= l_p[i]:
                    if o_p[i] > c_p[i]:
                        if c_p[i] >= h_v[i] and c_p[i] > l_p[i]:
                            print('type2 ',num)
                            return True
                    elif c_p[i] > o_p[i]:
                        if o_p[i] >= h_v[i] and o_p[i] > l_p[i]:
                            print('type2 ', num)
                            return True
        else:
            return False

def testBBands_2(num, macdList, bList, h_p, l_p, c_p, o_p, h_v, m_v, l_v):
    for i in range(len(h_v)):
        if i == 0:
            if h_p[i] < h_v[i] and l_p[i] > l_v[i] and h_p[i] >= m_v[i] >= l_p[i]:
                if c_p[i] <= o_p[i]:
                    if (l_p[i+1] > m_v[i+1] or o_p[i+1] > c_p[i+1]):
                        print('type3 ', num)
                        return True
        else:
            return False
    ...
    for line in lines:
        ...
        if typeName in line:
            ...
            if len(all_list) > 1:
                ...
                isFF = testBBands(num, all_list, bList, h_p, l_p, c_p, o_p, h_v, m_v, l_v)
                isFF_1 = testBBands_1(num, all_list, bList, h_p, l_p, c_p, o_p, h_v, m_v, l_v)
                isFF_2 = testBBands_2(num, all_list, bList, h_p, l_p, c_p, o_p, h_v, m_v, l_v)

                if percent > 0.5:
                    count_2 += 1
                if isTestFig and (isFF or isFF_1 or isFF_2) :
                    if isOutFig:
                        figPathName = ''
                        if isFF:
                            figPathName = 'hisBBands/type1'
                        elif isFF_1:
                            figPathName = 'hisBBands/type2'
                        elif isFF_2:
                            figPathName = 'hisBBands/type3'
                        drawFig.testDraw2Fig(num, all_list, bList, h_p, l_p, c_p, o_p, h_v, m_v, l_v, figPathName)
                if isDebug:
                    isPause = True
